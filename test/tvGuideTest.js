...
const tvGuideSearch = require('../commands/tvGuideSearch.js');

describe('tvGuide returns shows airing on the given date', () => {
  it('converts date to YYYY-MM-DD', () => {
    const returnMsg = tvGuide.formatDate(new Date(2019, 0, 1));
    expect(returnMsg).to.equal('2019-01-01');
  });
  it('return no shows found', async () => {
    const returnMsg = await tvGuideSearch('2019-02-01', []);
    expect(returnMsg).to.equal('no shows airing on 2019-02-01');
  });
